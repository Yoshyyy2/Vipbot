...
function isApproved(userId) {
  ...
  return loadDB().approvedUsers.includes(userId);
}
...
function runExpect(script) {
  return new Promise((resolve, reject) => {
    const tmp = `/tmp/vipbot_${Date.now()}.exp`;
...
bot.on('message', async (msg) => {
  ...
  if (mode === 'ssh') {
    ...
    try {
      ...
      bot.sendMessage(msg.chat.id, `❌ Failed to create SSH account. Please try again with /createssh`);
    }
    ...
    try {
      ...
      bot.sendMessage(msg.chat.id, `❌ Failed to create VLESS account. Please try again with /createvless`);
    }
  }
});
...
bot.onText(/\/request/, (msg) => {
  ...
  if (isApproved(userId)) {
    return bot.sendMessage(msg.chat.id, `✅ You already have access!\n\n🔑 /createssh — Create SSH account\n🌐 /createvless — Create VLESS account`);
  }
  const db = loadDB();
  ...
  bot.sendMessage(config.ADMIN_ID,
    `🔔 New Access Request\n━━━━━━━━━━━━━━━━━━━━\n👤 Name: ${msg.from.first_name}\n🆔 User ID: ${userId}\n📛 Username: @${msg.from.username || 'N/A'}\n━━━━━━━━━━━━━━━━━━━━\n/grantaccess_${requestId}\n/denyaccess_${requestId}`
  ).catch(() => {});
});

bot.onText(/\/createssh/, (msg) => {
  if (!isApproved(msg.from.id)) {
    return bot.sendMessage(msg.chat.id, `🚫 *You don't have access to this bot!*\n\nSend /request to ask for access.`, { parse_mode: 'Markdown' });
  }
...
bot.onText(/\/help/, (msg) => {
  ...
  if (isAdmin) {
    bot.sendMessage(msg.chat.id,
      ...
      `🗑️ /deleteaccount <username> — Remove account\n` +
    ...
    bot.sendMessage(msg.chat.id, `🚫 *You don't have access yet!*\n\nSend /request to ask for access.`, { parse_mode: 'Markdown' });
  }
});
...
bot.onText(/\/deleteaccount (.+)/, (msg, match) => {
  ...
  if (db.accounts.length < before) {
    bot.sendMessage(msg.chat.id, `✅ Account \`${username}\` removed.`, { parse_mode: 'Markdown' });
  } else {
...
console.log('🤖 Yosh VIP Bot is running...');
